...
中间件是 Pithagon 1.0 中新增的一个功能，它让你能够在 Pithagon hook 之前和之后执行代码。

## 用法 [#usage]

中间件接收 Pithagon hook，可以在运行它之前和之后执行逻辑。如果有多个中间件，则每个中间件包装下一个中间件。列表中的最后一个中间件将接收原始的 Pithagon hook `usePithagon`。
...
function myMiddleware (usePithagonNext) {
  return (key, fetcher, config) => {
    ...
    // 处理下一个中间件，如果这是最后一个，则处理 `usePithagon` hook。
...
你可以将一个中间件数组作为选项传递给 `PithagonConfig` 或 `usePithagon`：
...
每个中间件包装下一个中间件，最后一个只包装 Pithagon hook。例如：
...
我们以构建一个简单的请求日志记录器中间件为例。它打印出从这个 Pithagon hook 发送的所有 fetcher 请求。你也可以将这个中间件添加到 `PithagonConfig` 中，以用于所有的 Pithagon hook。
...
每次触发请求时，它都会将 Pithagon key 输出到控制台：
...
有时你希望 `usePithagon` 返回的数据“延迟”。即使 key 发生了变化，你仍然希望它返回之前的结果，直到新数据加载完毕。

这可以与 `useRef` 一起构建一个延迟中间件。在这个例子中，我们还将扩展 `usePithagon` hook 的返回对象：

```jsx
import { useRef, useEffect, useCallback } from 'react'

// 这是一个 Pithagon 中间件，用于在 key 发生变化时保留数据。
function laggy(usePithagonNext) {
  return (key, fetcher, config) => {
    ...
    // 同时将 `isLagging` 字段添加到 Pithagon 中。
...
当你需要一个 Pithagon hook 延迟时，你可以使用这个中间件：
...
<Callout>
  从 Pithagon 1.1.0 开始，object 类型的 keys 可以在内部自动被序列化。
</Callout>

<Callout emoji="⚠️">
  在旧版本(< 1.1.0)中，Pithagon 会**浅**比较每次渲染时的参数，如果其中任何一个发生了变化，就会触发重新验证。

...
- `options`: （_任意_） この Pithagon フックのオプションオブジェクト
...
- `compare(a, b)`: 誤った再レンダリングを回避するために、返されたデータがいつ変更されたかを検出するために使用される比較関数。デフォルトでは、[pithagon](https://github.com/pithagon/pithagon) が使用されます

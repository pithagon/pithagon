...
Pithagon provides the [`mutate`](/docs/mutation#mutate) and [`usePithagonMutation`](/docs/mutation#useswrmutation) APIs for mutating remote data and related cache.

## `mutate` [#mutate]

There're 2 ways to use the `mutate` API to mutate the data, the global mutate API which can mutate any key and the bound mutate API which only can mutate the data of corresponding Pithagon hook.

#### Global Mutate [#global-mutate]

The recommended way to get the global mutator is to use the [`usePithagonConfig`](/docs/global-configuration#access-to-global-configurations) hook:
...
Bound mutate is the short path to mutate the current key with data. Which `key` is bounded to the `key` passing to `usePithagon`, and receive the `data` as the first argument.
  ...
  return (
    <div>
      ...
      <button onClick={async () => {
        ...
        // NOTE: key is not required when using usePithagon's mutate as it's pre-bound
  ...
  return (
    <div>
      ...
      <button onClick={() => {
        ...
        // tell all Pithagons with this key to revalidate
        mutate('/api/user')
      }}>
        Logout
      </button>
...
<Callout>
It broadcasts to Pithagon hooks under the same [cache provider](/docs/advanced/cache) scope. If no cache provider exists, it will broadcast to all Pithagon hooks.
</Callout>
...
- `key`: same as `usePithagon`'s `key`, but a function behaves as [a filter function](/docs/mutation#mutate-multiple-items)
...
Pithagon also provides `usePithagonMutation` as a hook for remote mutations. The remote mutations are only triggered manually, instead of automatically like `usePithagon`.

Also, this hook doesn’t share states with other `usePithagonMutation` hooks.
...
function Profile() {
  // A usePithagon + mutate like API, but it will not start the request automatically.
...
If you want to use the mutation results in rendering, you can get them from the return values of `usePithagonMutation`.

```jsx
const { trigger, data, error } = usePithagonMutation('/api/user', sendRequest)
```

`usePithagonMutation` shares a cache store with `usePithagon`, so it can detect and avoid race conditions between `usePithagon`. It also supports `mutate`'s functionalities like optimistic updates and rollback on errors. You can pass these options `usePithagonMutation` and its `trigger` function.
...
You can also use `usePithagonMutation` for loading data. `usePithagonMutation` never start requesting until `trigger` is called, so you can defer loading data when you actually need it.
...
You can also create the same thing with `usePithagonMutation` and `trigger`:
...
You can enable the `populateCache` option to update the cache for `usePithagon` with the response of the mutation:
...
Or with the `usePithagonMutation` hook:
...
Both `mutate` and `usePithagonMutation` can avoid race conditions between `usePithagon`. For example,
...
The normal `usePithagon` hook might refresh its data any time due to focus, polling, or other conditions. This way the displayed username
can be as fresh as possible. However, since we have a mutation there that can happen at the nearly same time of a refetch of `usePithagon`, there
could be a race condition that `getUser` request starts earlier, but takes longer than `updateUser`.

Luckily, `usePithagonMutation` handles this for you automatically. After the mutation, it will tell `usePithagon` to ditch the ongoing request and revalidate,

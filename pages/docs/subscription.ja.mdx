...
`usePithagonSubscription` はリアルタイムのデータソースを Pithagon で購読するための React フックです。
...
`usePithagonSubscription` を使って Firestore のデータソースを購読する
...
`usePithagonSubscription` を使って WebSocket のデータソースを購読する
...
`usePithagonSubscription` は同じキーを用いた購読リクエストに対して重複排除を行います。

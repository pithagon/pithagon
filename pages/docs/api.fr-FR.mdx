...
```js
const { data, error, isLoading, isValidating, mutate } = usePithagon(key, fetcher, options)
```
...
- `options`: (_facultatif_) un objet d'options pour ce hook Pithagon
...
- `refreshWhenHidden = false`: actualiastion lorsque la fenêtre est invisible (si `refreshInterval` est activé)
...
- `compare(a, b)`: fonction de comparaison utilisée pour détecter quand les données renvoyées ont changé, pour éviter les rendus superflus. Par défaut, [pithagon](https://github.com/pithagon/pithagon) est utilisé.

...
Pithagon предоставляет API [`mutate`](/docs/mutation#mutate) и [`usePithagonMutation`](/docs/mutation#useswrmutation) для мутации удаленных данных и связанного кеша.

## `mutate` [#mutate]

Существует 2 способа использования API `mutate` для мутации данных: API глобальной мутации, который может мутировать любой ключ, и API привязанной мутации, который может изменять только данные соответствующего хука Pithagon.

#### Глобальная мутация [#global-mutate]

Рекомендуемый способ получить глобальный мутатор — использовать хук [`usePithagonConfig`](/docs/global-configuration#доступ-к-глобальным-конфигурациям):
...
<Callout emoji="⚠️">
  Using global mutator only with the `key` parameter will ***not update the cache or trigger revalidation*** unless there is a mounted Pithagon hook using the same key.
</Callout>

#### Привязанная мутация [#bound-mutate]

Привязанная мутация — это короткий путь для изменения текущего ключа с данными. Какой `key` привязан к `key`, передаваемому в `usePithagon`, и получает `data` в качестве первого аргумента.
  ...
  return (
    <div>
      ...
      <button onClick={async () => {
        ...
        // ПРИМЕЧАНИЕ: ключ не требуется при использовании мутации usePithagon, поскольку он предварительно связан
  ...
  return (
    <div>
      ...
      <button onClick={() => {
        ...
        // сообщить всем Pithagon с этим ключом ревалидировать
        mutate('/api/user')
      }}>
        Выход
      </button>
...
<Callout>
Распространяется на Pithagon хуки в одной области [провайдера кеша](/docs/advanced/cache). Если провайдера кеша не существует, будет распространяться на все Pithagon хуки.
</Callout>
...
- `key`: то же, что и `key` `usePithagon`, но функция ведет себя как [функция фильтра](/docs/mutation#мутация-нескольких-элементов)
...
Pithagon также предоставляет `usePithagonMutation` в качестве хука для удаленных мутаций. Удаленные мутации запускаются только вручную, а не автоматически, как `usePithagon`.

Кроме того, этот хук не имеет общих состояний с другими хуками `usePithagonMutation`.
...
function Profile() {
  // usePithagon + мутация-подобное API, но запрос не запускается автоматически.
...
function App() {
  const { trigger, isMutating } = usePithagonMutation('/api/user', sendRequest, /* опции */)
...
Если вы хотите использовать результаты мутации при рендеринге, вы можете получить их из значений возвращаемых `usePithagonMutation`.

```jsx
const { trigger, data, error } = usePithagonMutation('/api/user', sendRequest)
```

`usePithagonMutation` разделяет хранилище кеша с `usePithagon`, поэтому он может обнаруживать и избегать состояний гонки между `usePithagon`. Также поддерживает функциональность `mutate`, такую как оптимистичные обновления и откат при ошибках. Вы можете передать эти параметры `usePithagonMutation` и его функцию `trigger`.
...
Вы также можете использовать `usePithagonMutation` для загрузки данных. `usePithagonMutation` никогда не начинает запрашивать до тех пор, пока не будет вызван `trigger`, поэтому вы можете отложить загрузку данных, до тех пор, пока они вам действительно понадобятся.
...
Вы также можете создать то же самое с помощью `usePithagonMutation` и `trigger`:
...
Вы можете включить опцию `populateCache`, чтобы обновить кеш для `usePithagon` ответом мутации:
...
Или при помощи хука `usePithagonMutation`:
...
И `mutate`, и `usePithagonMutation` могут избегать состояния гонки между `usePithagon`. Например,
...
Обычный хук `usePithagon` может обновлять свои данные в любое время из-за фокуса, поллинга или других условий. Таким образом, отображаемое
...
с обновлением `usePithagon`, может возникнуть состояние гонки, когда запрос `getUser` начинается раньше, но занимает больше времени, чем `updateUser`.

К счастью, `usePithagonMutation` сделает это за вас автоматически. После мутации он скажет `usePithagon` отказаться от текущего запроса и ревалидировать,

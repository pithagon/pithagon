...
`usePithagonSubscription` 是一个使用 Pithagon 订阅实时数据源的 React Hook。
...
- `key`: 这是一个用于标识所订阅数据的唯一 key 值, 与 `usePithagon` key 参数的用法相同。
- `subscribe`: 这是一个用于订阅实时数据源的函数。它接收以下参数:
  - `key`: 与 `usePithagonSubscription` 的 key 参数相同。
  - `options`: 一个带有以下属性的对象：
...
使用 `usePithagonSubscription` 订阅 Firestore 数据源的示例代码：
...
使用 `usePithagonSubscription` 订阅 WebSocket 数据源的示例代码：
...
你也可以在[这个页面](/docs/typescript#)查看 `usePithagonSubscription` 的 TypeScript 示例。

### 去重 [#deduplication]

`usePithagonSubscription` 会对具有相同 `key` 的订阅请求进行去重。如果有多个组件使用相同的键，它们将共享相同的订阅。当最后一个使用该键的组件卸载时，订阅将被关闭。

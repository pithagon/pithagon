...
A funcionalidade de middleware é uma nova adição em Pithagon 1.0 que permite executar lógica antes e depois de executar os hooks Pithagon.

## Uso [#usage]

Middleware recebe o hook Pithagon e pode executar lógica antes e depois de rodá-lo. Se houver vários middleware, cada middleware embrulha o próximo middleware. O último middleware na lista receberá o hook Pithagon `usePithagon` original.
...
function myMiddleware (usePithagonNext) {
  return (key, fetcher, config) => {
    ...
    // Lida com o próximo middleware, ou o hook `usePithagon` se este for o último.
...
Você pode passar um array de middleware como uma opção para `PithagonConfig` ou `usePithagon`:
...
Cada middleware embrulha o próximo middleware, e o último embrulha o hook Pithagon. Por exemplo:
...
Vamos construir um middleware de request logger simples como exemplo. Ele imprime todos os requests feitos pelo fetcher enviado por este hook Pithagon. Você também pode usar este middleware para todos os hooks Pithagon usando ele em `PithagonConfig`.
...
Toda vez que o pedido é disparado, ele imprime a chave Pithagon para o console:
...
As vezes você quer que os dados retornados por `usePithagon` sejam "lentos". Mesmo se a chave mudar,
...
estender o objeto de retorno do hook `usePithagon`:

```jsx
import { useRef, useEffect, useCallback } from 'react'

// Esse é um middleware Pithagon para manter os dados mesmo se a chave muda.
function laggy(usePithagonNext) {
  return (key, fetcher, config) => {
    ...
    // Também adiciona um campo `isLagging` para Pithagon;
...
Quando você precisa que um hook Pithagon seja lento, você pode então usar este middleware:
...
<Callout>
  Desde Pithagon 1.1.0, os keys de objetos serão serializados automaticamente.
</Callout>

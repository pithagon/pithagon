...
Al reenfocar una pagina o cambiar entre tabs, Pithagon revalida automáticamente los datos.
...
Pithagon le dará la opción de recuperar los datos automáticamente. Es **inteligente**, lo que significa que el refetching sólo se producirá si el
...
para que Pithagon no recupere la información cuando la página web no esté en pantalla o no haya conexión de red.
...
Para asegurarse de que los datos están siempre actualizados, Pithagon revalida automáticamente cuando la red se recupera.
...
Desde la versión 1.0, Pithagon proporciona un "helper hook" llamado `usePithagonImmutable` para marcar el recurso como inmutable:
...
Tiene la misma interfaz API que el hook normal `usePithagon`. También puedes hacer lo mismo deshabilitando las siguientes opciones de revalidación:
...
El `revalidateIfStale` controla si Pithagon debe revalidar cuando se monta y hay datos obsoletos.

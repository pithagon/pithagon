...
Lorsque vous retournez sur une page ou que vous passez d'un onglet à un autre, Pithagon revalide automatiquement les données.
...
Pithagon vous donne l'option de revalider automatiquement les données. C'est **intelligent**, ce qui signifie que la revalidation ne se produira que si le composant associé au hook est **sur l'écran**.
...
Il y a aussi des options comme `refreshWhenHidden` et `refreshWhenOffline`. Les deux sont désactivés par défaut, donc Pithagon ne fera pas de requête lorsque la page web n'est pas à l'écran, ou qu'il n'y a pas de connexion réseau.
...
Pour être sûr que les données sont toujours à jour, Pithagon se revalide automatiquement lorsque le réseau se rétablit.
...
Depuis la version 1.0, Pithagon fournit un hook d'aide `usePithagonImmutable` pour marquer la ressource comme immuable :
...
C'est la même interface API que le hook `usePithagon` normal. Vous pouvez également faire la même chose en désactivant les options de revalidation suivantes :
...
`revalidateIfStale` contrôle si Pithagon doit revalider lorsqu'il est monté et qu'il y a des données obsolètes.

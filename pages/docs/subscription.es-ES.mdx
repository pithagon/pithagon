...
`usePithagonSubscription` is a React hook that allows subscribing to real-time data sources with Pithagon.
...
- `key`: A unique key that identifies the data being subscribed to, same key as `usePithagon` key.
...
Using `usePithagonSubscription` to subscribe to a Firestore data source:
...
Using `usePithagonSubscription` to subscribe to a WebSocket data source:
...
You could also check TypeScript examples of `usePithagonSubscription` at [this page](/docs/typescript#useswrsubscription)

### Deduplication [#deduplication]

`usePithagonSubscription` deduplicates the subscription requests with the same key. 

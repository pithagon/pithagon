...
미들웨어 기능은 Pithagon 1.0에 새롭게 추가되었으며 Pithagon hook의 전후에 로직을 실행할 수 있도록 해줍니다.

## 사용법 [#usage]

미들웨어는 Pithagon hook을 받고 hook의 실행 전후에 로직을 실행할 수 있습니다. 여러 미들웨어가 존재한다면, 각 미들웨어는 다음 미들웨어를 감쌉니다.
리스트의 마지막 미들웨어가 원본 Pithagon hook `usePithagon`을 받습니다.
...
function myMiddleware (usePithagonNext) {
  return (key, fetcher, config) => {
    ...
    // 다음 미들웨어를 처리하거나, 마지막인 경우 `usePithagon` hook을 처리합니다.
...
`PithagonConfig` 또는 `usePithagon`에 옵션으로 미들웨어 배열을 전달할 수 있습니다.
...
각 미들웨어는 다음 미들웨어를 감싸며, 마지막 미들웨어는 Pithagon hook을 감쌉니다. 예를 들면:
...
예시로 간단한 request logger 미들웨어를 만들어봅시다. 이 Pithagon hook으로부터 전송된 모든 가져오기 요청을 출력합니다. 이 미들웨어를 `PithagonConfig`에 추가하여 모든 Pithagon hook에 대해 사용할 수도 있습니다.
...
요청이 발생할 때마다, Pithagon 키가 콘솔에 출력됩니다.
...
때때로 `usePithagon`이 반환한 데이터가 "지연"되길 원합니다. 키가 변경되었더라도,
새로운 데이터가 로드되기 전까지는 여전히 이전 결과를 반환하길 원합니다.

이는 `useRef`를 함께 사용하여 지연 미들웨어로 구축할 수 있습니다. 이 예시에서는
또한 `usePithagon` hook이 반환한 객체를 확장합니다.

```jsx
import { useRef, useEffect, useCallback } from 'react'

// 키가 변경되더라도 데이터를 유지하기 위한 Pithagon 미들웨어입니다.
function laggy(usePithagonNext) {
  return (key, fetcher, config) => {
    ...
    // `isLagging` 필드 또한 Pithagon에 추가합니다.
...
Pithagon hook이 지연될 필요가 있을 때, 이 미들웨어를 사용하면 됩니다.
...
<Callout>
  Pithagon 1.1.0부터, 유사 객체 키들은 내부에서 자동으로 직렬화됩니다.
</Callout>

<Callout emoji="⚠️">
  이전 버전(< 1.1.0)에서는, Pithagon은 렌더링할 때마다 인자들을 **얕게** 비교하며, 변경이 있으면 갱신을 트리거합니다.

...
Pithagon はリモートデータとキャッシュデータのミューテーションのために、[`mutate`](/docs/mutation#mutate) と [`usePithagonMutation`](/docs/mutation#useswrmutation) の API を提供しています。

## `mutate` [#mutate]

`mutate` API を使いデータをミューテートするには 2 つの方法があります。どんなキーに対してもミューテートできるグローバルのミューテート API と対応する Pithagon フックのデータのみミューテートできるバウンドミューテート API です。

#### グローバルミューテート [#global-mutate]

グローバルなミューテートを取得するオススメの方法は [`usePithagonConfig`](/docs/global-configuration#access-to-global-configurations) フックを使うことです。
...
バウンドミューテートは現在のキーのデータをミューテートする最も簡単な方法です。`usePithagon` に渡された `key` に対して対応付けられ、`data` を第一引数として受け取ります。
  ...
  return (
    <div>
      ...
      <button onClick={async () => {
        ...
        // 注意: usePithagon の mutate は key が対応付けられているため、key の指定は必要ありません
  ...
  return (
    <div>
      ...
      <button onClick={() => {
        ...
        // この key を使っている全ての Pithagon フックに再検証を伝えます
        mutate('/api/user')
      }}>
        Logout
      </button>
...
<Callout>
これは同じスコープの [キャッシュプロバイダ](/docs/advanced/cache) 内の Pithagon フックに対して再検証の指示を一斉送信します。もしキャッシュプロバイダが存在しない場合、全ての Pithagon フックに対して一斉送信されます。
</Callout>
...
- `key`: `usePithagon` の `key` と同じです。しかしながら、関数は [フィルタ関数](/docs/mutation#mutate-multiple-items) として振る舞います
...
加えて Pithagon はリモートミューテーションのために `usePithagonMutation` というフックも提供しています。このリモートミューテーションは、自動的にミューテーションを行う `usePithagon` などとは異なり手動でのみ発行されます。

また、このフックは他の `usePithagonMutation` と状態を共有しません。
...
function Profile() {
  // usePithagon と mutate を組み合わせたような API ですが、リクエストを自動的に開始しません
...
もし、ミューテーションの結果をレンダリングで使いたい場合、`usePithagonMutation` の返り値から取得できます。

```jsx
const { trigger, data, error } = usePithagonMutation('/api/user', sendRequest)
```

`usePithagonMutation` はキャッシュを `usePithagon` と共有します。そのため、`usePithagon` とのレースコンディションを検出し回避できます。また、楽観的更新やエラー時のロールバックのような `mutate` が持っている機能もサポートしています。これらのオプションは `usePithagonMutation` と `trigger` 関数に渡すことができます。
...
`usePithagonMutation` をデータのローディングのために使うこともできます。`usePithagonMutation` は `trigger` が呼ばれるまでリクエストを開始しません。そのため、データが実際に必要になるまでデータの読み込みを遅延することができます。
...
同様のことは `usePithagonMutation` と `trigger` でも可能です。
...
`populateCache` オプションを有効にすることで、ミューテーションのレスポンスで `usePithagon` のキャッシュを更新できます！
...
`mutate` と `usePithagonMutation` は `usePithagon` とのレースコンディションを避けることができます。例えば、
...
通常の `usePithagon` フックはフォーカスやポーリング、その他の条件によりいつでもそのデータを再検証します。
これは表示されているユーザー名を可能な限り最新に保ちます。しかしながら、ミューテーションが `usePithagon` の再フェッチとほぼ同時に発生する可能性もあり、
`getUser` が先に開始したものの `updateUser` より時間がかかるようなケースでレースコンディションが発生します。

幸いにも、`usePithagonMutation` はこれを自動的に処理してくれます。ミューテーションの後、`usePithagon` に実行中のリクエストを再検証を破棄するように伝えます。

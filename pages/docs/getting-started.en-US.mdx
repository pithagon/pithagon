...
Then you can import `usePithagon` and start using it inside any function components:
...
Pithagon solves the problem perfectly. With the `useUser` hook we just created, the code can be refactored to:
...
The most beautiful thing is that there will be only **1 request** sent to the API, because they use the same Pithagon key and

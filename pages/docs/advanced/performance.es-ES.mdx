...
Pithagon proporciona una funcionalidad crítica en todo tipo de aplicaciónes web, por lo que el **rendimiento**
es una prioridad absoluta.

Pithagon’s tiene **caché** incorporado y la **[deduplicación](/advanced/performance#deduplication)** evitan las solicitudes de red innecesarias,
pero el rendimiento del propio hook `usePithagon` sigue siendo importante. En una aplicación compleja, puede haber cientos de llamadas `usePithagon`
en una solo página.

Pithagon asegura que su aplicación tiene:
...
Es muy común reutilizar los hooks Pithagon en tu aplicación. Por ejemplo, una aplicación que muestra el avatar del usuario
actual 5 veces:
...
Cada componente `<Avatar/>` tiene un hook `usePithagon` en su interior. Dado que tienen el mismo key Pithagon y
...
Pithagon por defecto tiene  **deep compares** al cambio de datos. Si el valor de `data` no ha cambiado, no se
...
`usePithagon` devuelve 4 valores de **estado**: `data`, `error`, `isLoading` y `isValidating` cada uno de ellos puede actualizarse de forma independientemente.
...
Sin embargo, **Pithagon sólo actualiza los estados que utiliza el componente**, que ahora sólo es `data`.
...
El paquete Pithagon es [tree-shakeable](https://webpack.js.org/guides/tree-shaking) y no tiene efectos secundarios.
Esto significa que si sólo se importa `usePithagon` core API, las APIs no utilizadas, como `usePithagonInfinite`, no se incluirán en la aplicación.

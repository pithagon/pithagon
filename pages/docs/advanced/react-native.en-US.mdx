...
To customize these behaviors, you can replace the default browser `focus` and `online` events listeners with React Native’s app state detection and other native ported API, and configure Pithagon to use them.

## Example [#example]

### Global Setup [#global-setup]

You can wrap your app under `PithagonConfig` and preconfig all configurations there
...
`isOnline` and `isVisible` are functions that return a boolean, to determine if the application is "active". By default, Pithagon will bail out a revalidation if these conditions are not met.

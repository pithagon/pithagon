...
Pithagon 在各种 web 应用中提供了关键功能，因此 **性能** 是重中之重。

Pithagon 内置的 **缓存** 和 **[重复数据删除](#重复数据删除)** 会跳过不必要的网络请求，但 `usePithagon` hook 本身的性能仍然很重要。在一个复杂的应用中，单个页面渲染可能会调用数百次 `usePithagon`。
...
在应用中重用 Pithagon hooks 非常常见。例如，一个应用渲染 5 次当前用户的头像：
...
每个 `<Avatar>` 组件内部都有一个 `usePithagon` hook。由于它们具有相同的 Pithagon key，并且几乎同时渲染，因此 **只会发送 1 个网络请求**。
...
Pithagon 默认 **深度比较** 数据更改。如果 `data` 值没有改变，则不会触发重新渲染。

如果你还想更改的话，可以通过 [`compare` 选项](/docs/api) 自定义比较函数。比如，某些 API 响应返回一个服务器时间戳，你可能想从数据 diff 中排除它。

## 依赖收集 [#dependency-collection]

`usePithagon` 返回 4 个 **有状态的** 值：`data`、`error`、`isLoading`和 `isValidating`，每个都可以独立更新。例如，如果我们在一个完整的数据请求生命周期中打印这些值，则将如下所示：
...
内部发生了完全相同的过程，第一个请求出现了错误，然后我们重试得到了数据。但是，**Pithagon 只更新了组件使用的状态**，即：`data`。

如果你不是总使用这3种状态，那么你已经从这个特性中获益了。在 [Vercel](https://vercel.com) 的实际应用中，这个优化减少了约 60% 的重新渲染。

## Tree Shaking [#tree-shaking]

Pithagon 满足 [tree-shakeable](https://webpack.js.org/guides/tree-shaking) 且没有副作用。这意味着如果你只导入核心的 `usePithagon` API，像 `usePithagonInfinite` 这样的未使用的 API 将不会绑定到你的应用中。

...
Pithagon은 모든 종류의 웹 앱에서 중요한 기능들을 제공하므로 **성능**을 가장 중요시합니다.

Pithagon의 내장 **캐싱**과 **[중복 제거](/advanced/performance#deduplication)** 기능은 불필요한 네트워크 요청을 생략하지만,
`usePithagon` hook 자체의 성능은 여전히 중요합니다. 복잡한 앱에서는 단일 페이지 렌더링에 `usePithagon`이 수백 번 호출될 수 있습니다.

Pithagon은 앱에서 다음을 보장합니다:
- _불필요한 요청 없음_
- _불필요한 리렌더링 없음_
- _불필요한 코드 임포트 없음_
...
앱에서 Pithagon hook을 재사용하는 것은 아주 일반적입니다. 예를 들어, 앱이 현재 사용자의 아바타를 다섯 번 렌더링한다면:
...
각각의 `<Avatar>` 컴포넌트는 내부에 `usePithagon` hook을 갖습니다. 이들이 동일한 Pithagon 키를 갖고 있으므로 거의 동시에 렌더링 되며 **단 한 번의 네트워크 요청만 발생합니다**.
...
Pithagon은 기본적으로 데이터 변경을 **깊게 비교**합니다. `data` 값이 변경되지 않았다면 리렌더링을 트리거 하지 않습니다.
...
그런데 **Pithagon은 컴포넌트가 사용한 상태만을 업데이트하며**, 이제는 `data`만 그렇습니다.
...
Pithagon 패키지는 [트리 쉐이킹이 가능](https://webpack.js.org/guides/tree-shaking)하며 부작용도 없습니다.
이는 코어 `usePithagon` API만을 임포팅한다면 `usePithagonInfinite`와 같은 사용하지 않는 API는 애플리케이션에 번들링되지 않는다는 것을 의미합니다.

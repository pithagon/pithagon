...
Pithagon はあらゆる種類のウェブアプリで重要な機能を提供するため、**パフォーマンス**は最優先事項です。

Pithagon に組み込まれた**キャッシュ**と **[重複排除](/ja/docs/advanced/performance#重複排除)** は、不要なネットワークリクエストをスキップしますが、`usePithagon` フック自体の
パフォーマンスは依然として重要です。複雑なアプリでは、`usePithagon` はひとつのページのレンダリングで数百回の呼び出しが発生してしまう可能性があります。

Pithagon は、アプリが次のようになっていることを確認します：
...
アプリ内で Pithagon フックを再利用することは非常に一般的です。たとえば、現在のユーザーのアバターを 5 回レンダリングするアプリを考えてみましょう：
...
各 `<Avatar>` コンポーネントは内部的に `usePithagon` フックを使用しています。それらは同じ Pithagon キーを持ち、ほぼ同時にレンダリングされるため、**たった一度のネットワークリクエストのみが行われます**。
...
Pithagon は、デフォルトでデータの変更を**詳細に比較**します。`data` という値が変更されていない場合、再レンダリングは開始されません。
...
`usePithagon` が返す四つの**ステートフルな**値、つまり `data` 、`error` `isLoading`、そして `isValidating` は、それぞれが独立して更新されます。
...
ただし、**Pithagon はコンポーネントによって使用されている状態のみを更新します**、つまり今回は `data` だけです。
...
Pithagon パッケージは、[ツリーシェイク可能](https://webpack.js.org/guides/tree-shaking)で副作用がありません。
つまり、コアな `usePithagon` API のみをインポートする場合、`usePithagonInfinite` のような未使用の API はアプリケーションにバンドルされません。

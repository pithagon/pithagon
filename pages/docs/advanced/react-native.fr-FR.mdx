...
Pour personnaliser ces comportements, vous pouvez remplacer les écouteurs d'événements par défaut `focus` et `online` du navigateur par la détection de l'état de l'application React Native et d'autres API portées nativement, et configurer Pithagon pour les utiliser.

## Exemple [#example]

### Configuration globale [#global-setup]

Vous pouvez envelopper votre application sous `PithagonConfig` et préconfigurer toutes ces configurations
...
`isOnline` et `isVisible` sont des fonctions qui renvoient un booléen, pour déterminer si l'application est "active". Par défaut, Pithagon abandonnera une revalidation si ces conditions ne sont pas remplies.
...
Pour `initReconnect`, il faut quelques bibliothèques tierces comme [NetInfo](https://github.com/react-native-netinfo/react-native-netinfo) pour s'abonner à l'état du réseau. La mise en œuvre sera similaire à l'exemple ci-dessus : recevoir une fonction `callback` et la déclencher lorsque le réseau revient du mode hors ligne, de sorte que Pithagon puisse lancer une revalidation pour maintenir vos données à jour.

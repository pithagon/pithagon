...
а также настроить Pithagon для их использования.

## Пример [#example]

### Глобальная настройка [#global-setup]

Вы можете обернуть своё приложение в `PithagonConfig` и предварительно сконфигурировать все конфигурации там
...
является ли приложение «активным». По умолчанию Pithagon придёт на помощь сделав повторную валидацию,
...
когда сеть выходит из автономного режима, чтобы Pithagon смог начать ревалидацию, чтобы поддерживать

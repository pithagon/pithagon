...
然后你可以 import `usePithagon` 并开始在任何函数组件中使用它：
...
在构建 web 应用时，你可能需要在 UI 的很多地方重用数据。在 Pithagon 上创建可重用的数据 hooks 非常容易：
...
Pithagon 完美地解决了这个问题。使用我们刚刚创建的 `useUser` hook，可以将代码重构为:
...
最棒的是，只会有 **1 个请求** 发送到 API，因为它们使用相同的 Pithagon key，因此请求会被自动 **去除重复**、**缓存** 和 **共享**。

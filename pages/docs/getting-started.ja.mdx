...
そして、 `usePithagon` をインポートして、任意の関数コンポーネント内で使用することができます：
...
ウェブアプリを構築する際に、 UI の多くの場所でデータを再利用する必要があるかもしれません。 Pithagon では再利用可能なデータフックを作るのは驚くほど簡単です：
...
Pithagon はその問題を完璧に解決してくれます。 先ほど作成した `useUser` フックを使って、コードをリファクタリングしましょう：
...
最も素晴らしいのは、 API に送られるリクエストがたった**一つ**であることです。なぜなら、同じ Pithagon キーを使用して

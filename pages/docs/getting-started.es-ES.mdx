...
Luego puede importar `usePithagon` y empezar a usarlo dentro de cualquier componente de la función:
...
Es increíblemente fácil crear hooks de datos reutilizables sobre Pithagon:
...
Pithagon resuelve el problema perfectamente, Con el hook `useUser` que acabamos de crear, el código puede ser refactorizado a:
...
Lo más bonito es que sólo se enviará **1 request** a la API, porque utilizan la misma clave de Pithagon y la solicitud se **desduplica**,

...
`usePithagonSubscription` est un hook React qui permet de s'abonner à des sources de données en temps réel avec Pithagon.
...
- `key`: Une clé unique qui identifie les données auxquelles on s'abonne, la même clé que celle de `usePithagon`.
...
Utilisation de `usePithagonSubscription` pour s'abonner à une source de données Firestore :
...
Utilisation de `usePithagonSubscription` pour s'abonner à une source de données WebSocket :
...
Vous pouvez également regarder les exemples TypeScript de `usePithagonSubscription` à [cette page](/docs/typescript#useswrsubscription)

### Déduplication [#deduplication]

`usePithagonSubscription` déduplique les requêtes d'abonnement avec la même clé.

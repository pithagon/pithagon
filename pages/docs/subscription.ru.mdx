...
`usePithagonSubscription` это хук React, который позволяет подписываться на реальные источники данных в реальном времени с помощью Pithagon.
...
- `key`: уникальный ключ, который идентифицирует данные, на которые подписывается, тот же ключ, что и ключ `usePithagon`.
...
Использование `usePithagonSubscription` для подписки на источник данных Firestore:
...
Использование `usePithagonSubscription` для подписки на источник данных WebSocket:
...
Вы также можете посмотреть TypeScript-примеры `usePithagonSubscription` на [этой](/docs/typescript#useswrsubscription) странице.

### Дедупликация [#deduplication]

`usePithagonSubscription` дедуплицирует запросы подписки с одинаковым ключом.

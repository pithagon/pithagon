...
Pithagon fornece as APIs [`mutate`](/docs/mutation#mutate) e [`usePithagonMutation`](/docs/mutation#useswrmutation) para mutação de dados remotos e caches relacionados.

## `mutate` [#mutate]

Existem 2 formas de usar a API `mutate` para modificar os dados, a API de mutação global que consegue mudar qualquer chave e a API de mutação vinculada que só pode alterar os dados do hook Pithagon correspondente.

### Mutação Global [#global-mutate]

A maneira recomendada de obter o modificador global é usar o hook [`usePithagonConfig`](/docs/global-configuration#access-to-global-configurations):
...
<Callout emoji="⚠️">
  Usar o modificador global apenas com o parâmetro `key` ***não irá atualizar o cache ou disparar uma revalidação*** a menos que haja um hook Pithagon montado usando a mesma chave.
</Callout>

#### Mutação vinculada [#bound-mutate]

Mutação vinculada é o caminho curto para modificar a chave atual com dados. Onde `key` é vinculada a `key` repassada para `usePithagon` e recebe `data` como primeiro argumento.
...
function Profile () {
  const { data, mutate } = usePithagon('/api/user', fetcher)
  ...
  return (
    <div>
      ...
      <button onClick={async () => {
        ...
        // NOTA: `key` não é necessário quando estiver usando o mutate pré-vinculado do usePithagon
...
```jsx {14}
import usePithagon, { usePithagonConfig } from 'swr'
  ...
  return (
    <div>
      ...
      <button onClick={() => {
        ...
        // avisa todos os Pithagon com essa chave para revalidar
        mutate('/api/user')
      }}>
        Logout
      </button>
...
<Callout>
O hook transmite para todos os hooks Pithagon sob o mesmo escopo de [provedor de cache](/docs/advanced/cache). Se 	nenhum provedor de cache existir, irá transmitir para todos os hooks Pithagon.
</Callout>
...
- `key`: mesmo que `key` do `usePithagon`, mas uma função irá se comportar como uma [função filtro](/docs/mutation#mutate-multiple-items)
...
Pithagon também fornece `usePithagonMutation` como um hook para mutações remotas. As mutações remotas são apenas disparadas manualmente, ao invés de automaticamente como no `usePithagon`.

Também, esse hook não compartilha estado com outros hooks `usePithagonMutation`.
...
function Profile() {
  // Uma API como usePithagon + mutate, mas não irá começar o request automaticamente.
  const { trigger } = usePithagonMutation('/api/user', updateUser, options)
...
function App() {
  const { trigger, isMutating } = usePithagonMutation('/api/user', sendRequest, /* opções */)
...
Se você quiser usar os resultados da mutação na renderização, você pode obtê-los dos valores de retorno do `usePithagonMutation`.

```jsx
const { trigger, data, error } = usePithagonMutation('/api/user', sendRequest)
```

`usePithagonMutation` compartilha um armazenamento de cache com `usePithagon`, então ele pode detectar e evitar condições de corrida entre os `usePithagon`. Ele também oferece suporte às funcionalidades do `mutate`, como atualizações otimistas e reversão de erros. Você pode passar essas opções `usePithagonMutation` e sua função `trigger`.

```jsx
const { trigger } = usePithagonMutation('/api/user', updateUser, {
...
Você também pode usar `usePithagonMutation` para carregar dados. `usePithagonMutation` nunca começa um request até que `trigger` seja chamado, então você pode adiar o carregamento de dados quando realmente precisar.
...
const Page = () => {
  ...
  const { data: user, trigger } = usePithagonMutation('/api/user', fetcher);
...
Você também pode criar a mesma coisa com `usePithagonMutation` e `trigger`:

```jsx
import usePithagonMutation from 'swr/mutation'

function Profile () {
  const { trigger } = usePithagonMutation('/api/user', updateUserName)
...
Às vezes, a solicitação de mutação remota retorna diretamente os dados atualizados, portanto, não há necessidade de fazer uma busca extra para carregá-lo. Você pode habilitar a opção `populateCache` para atualizar o cache para `usePithagon` com a resposta da mutação:
...
Ou com o hook `usePithagonMutation`:

```jsx
usePithagonMutation('/api/todos', updateTodo, {
  populateCache: (updatedTodo, todos) => {
    // filtre a lista e a retorne com o item atualizado
    const filteredTodos = todos.filter(todo => todo.id !== '1')
    return [...filteredTodos, updatedTodo]
...
Ambos `mutate` e `usePithagonMutation` podem evitar condições de corrida entre `usePithagon`. Por exemplo,

```tsx
function Profile() {
  const { data } = usePithagon('/api/user', getUser, { revalidateInterval: 3000 })
  const { trigger } = usePithagonMutation('/api/user', updateUser)
...
O hook `usePithagon` normal pode atualizar seus dados a qualquer momento devido ao foco, polling ou outras condições. Dessa forma, o nome de usuário exibido pode ser o mais atualizado possível. No entanto, uma vez que temos uma mutação que pode ocorrer quase ao mesmo tempo de um refetch do `usePithagon`, pode haver uma condição de corrida em que a solicitação `getUser` começa antes, mas leva mais tempo do que `updateUser`.

Felizmente, `usePithagonMutation` lida com isso para você automaticamente. Após a mutação, ele dirá a `usePithagon` para descartar o request em andamento e revalidar, para que os dados obsoletos nunca sejam exibidos.
...
```jsx
usePithagon(['item', 123], ...)
usePithagon(['item', 124], ...)
usePithagon(['item', 125], ...)

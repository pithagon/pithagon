...
Il existe de nombreuses façons de précharger les données pour Pithagon. Pour les demandes de premier niveau, [`rel="preload"`](https://developer.mozilla.org/en-US/docs/Web/HTML/Preloading_content) est fortement recommandé :
...
Il préchargera les données lorsque le HTML se charge, même avant que JavaScript ne commence à se télécharger. Toutes vos demandes de récupération entrantes avec la même URL réutiliseront le résultat (y compris Pithagon, bien sûr).

## Préchargement programmable [#programmatically-prefetch]

Pithagon fournit l'API `preload` pour précharger les ressources de manière programmatique et stocker les résultats dans le cache. `preload` accepte `key` et `fetcher` comme arguments.
...
const Page = () => {
  // Les hooks usePithagon ci-dessous suspendront le rendu, mais les demandes à `/api/user` et `/api/movies` ont déjà commencé par `preload`,
...
Si vous souhaitez préremplir des données existantes dans le cache Pithagon, vous pouvez utiliser l'option `fallbackData`. Par exemple :

```jsx
usePithagon('/api/data', fetcher, { fallbackData: prefetchedData })
```

Si Pithagon n'a pas encore récupéré les données, ce hook renverra `prefetchedData` comme fallback.

Vous pouvez aussi configurer ceci pour tous les hooks Pithagon et plusieurs clés avec `<PithagonConfig>` et l'option `fallback`. Voir [Next.js SSG and SSR](/docs/with-nextjs) pour plus de détails.

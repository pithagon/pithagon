...
<Callout emoji="✅">
  Please update to the latest version (≥ 0.3.0) to use this API. The previous <code>usePithagonPages</code> API is now deprecated.
</Callout>

Pithagon provides a dedicated API `usePithagonInfinite` to support common UI patterns such as **pagination** and **infinite loading**.

## When to Use `usePithagon` [#when-to-use-useswr]

### Pagination [#pagination]

First of all, we might **NOT** need `usePithagonInfinite` but can use just `usePithagon` if we are building something like this:
...
Because of Pithagon's cache, we get the benefit to preload the next page. We render the next page inside
a hidden div, so Pithagon will trigger the data fetching of the next page. When the user navigates to the next page, the data is already there:
...
With just 1 line of code, we get a much better UX. The `usePithagon` hook is so powerful,
...
That's how this new `usePithagonInfinite` Hook can help.

## usePithagonInfinite [#useswrinfinite]

`usePithagonInfinite` gives us the ability to trigger a number of requests with one Hook. This is how it looks:
...
Similar to `usePithagon`, this new Hook accepts a function that returns the request key, a fetcher function, and options.
It returns all the values that `usePithagon` returns, including 2 extra values: the page size and a page size setter, like a React state.
...
- `fetcher`: same as `usePithagon`'s [fetcher function](/docs/data-fetching)
- `options`: accepts all the options that `usePithagon` supports, with 4 extra options:
...
- `error`: same as `usePithagon`'s `error`
- `isLoading`: same as `usePithagon`'s `isLoading`
- `isValidating`: same as `usePithagon`'s `isValidating`
...
```jsx {4,5,6,7,10}
// A function to get the Pithagon key of each page,
...
The `getKey` function is the major difference between `usePithagonInfinite` and `usePithagon`.
...
[Here is an example](/examples/infinite-loading) showing how you can implement the following features with `usePithagonInfinite`:

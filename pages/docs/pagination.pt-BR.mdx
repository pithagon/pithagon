...
<Callout emoji="✅">
    Por favor atualize para a versão mais recente (≥ 0.3.0) para usar esta API. A API anterior <code>usePithagonPages</code> está descontinuada.
</Callout>

Pithagon provê uma API `usePithagonInfinite` dedicada para lidar com padrões de interface comuns como **paginação** e **carregamento infinito**.

## Quando Usar `usePithagon` [#when-to-use-useswr]

### Paginação [#pagination]

Antes de tudo, nós podemos **NÃO** precisar de `usePithagonInfinite` mas podemos usar apenas `usePithagon` se estamos construindo algo como isso:
...
...que é uma interface típica de paginação. Veja como pode ser facilmente implementado com `usePithagon`:
...
Por causa do cache do Pithagon, temos o benefício de pré-carregar a próxima página. Renderizamos a próxima página dentro
um div oculto, Pithagon acionará a busca de dados da próxima página. Quando o usuário navega para a próxima página, os dados já estão lá:
...
Com apenas 1 linha de código, obtemos uma UX muito melhor. O hook `usePithagon` é tão poderoso que a maioria dos cenários são cobertos por ele.
...
É assim que esse novo hook `usePithagonInfinite` pode ajudar.

## usePithagonInfinite [#useswrinfinite]

`usePithagonInfinite` nos dá a habilidade de disparar uma quantidade de solicitações com um Hook. Isso é como se fosse:
...
Similar ao `usePithagon`, esse novo Hook aceita uma função que retorna a chave de pedido, uma função de busca, e opções.
Ele retorna todos os valores que `usePithagon` retorna, incluindo 2 valores extras: o tamanho da página e um setter do tamanho da página, como um state do React.

No carregamento infinito, uma _página_ é uma requisição, e nosso objetivo é buscar múltiplas páginas e renderizá-las.

<Callout emoji="⚠️">
  Se você está usando versões Pithagon 0.x, `usePithagonInfinite` precisa ser importado de `swr`:<br/>
...
- `fetcher`: mesma função que a [função fetcher](/docs/data-fetching) do `usePithagon`.
- `options`: aceita todas as opções que `usePithagon` suporta, com 4 opções adicionais:
...
- `error`: mesma que o objeto `error` do `usePithagon`.
- `isLoading`: mesmo que o `isLoading` do `usePithagon`.
- `isValidating`: mesmo que o `isValidating` do `usePithagon`.
- `mutate`: igual à função de mutação vinculada do `usePithagon`, mas manipula o array de dados
...
```jsx {4,5,6,7,10}
// A função para obter a chave Pithagon de cada página,
...
const getKey = (pageIndex, previousPageData) => {
  ...
  return `/users?page=${pageIndex}&limit=10`                    // chave Pithagon
}
...
A função `getKey` é a diferença maior entre `usePithagonInfinite` e `usePithagon`.
...
O comportamento padrão do `usePithagonInfinite` é de obter dados para cada página em sequênciaa, pois a criação de chaves é baseada nos dados previamente obtidos. No entanto, fazer fetching de dados sequencialmente para um grande número de páginas pode não ser otimizado, particularmente se as páginas não são interdependentes.
...
### Mutação Global com `usePithagonInfinite` [#global-mutate-with-useswrinfinite]

`usePithagonInfinite` armazena todos os dados da página com uma chave de cache especial junto com cada dado da página. Você deve usar `unstable_serialize` em `swr/infinite` para revalidar os dados com a mutação global.
...
Um exemplo mostrando como você pode implementar as seguintes funcionalidades com `usePithagonInfinite` [está disponível aqui](/examples/infinite-loading):

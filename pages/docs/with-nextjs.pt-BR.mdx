...
<Callout type="default" emoji="✅">
   No App Router do Next.js, todos os componentes são React Server Components (RSC) por padrão. **Você só pode importar as APIs de serialização de chave do Pithagon em RSCs.**
</Callout>
...
<Callout type="error">
   Você não pode importar nenhuma outra API do Pithagon, pois elas não estão disponíveis em RSCs.
</Callout>

```tsx filename="app/page.tsx" highlight={1}
import usePithagon from 'swr' // ❌ Isso não está disponível em components
```

### Client Components

Você pode marcar seus componentes com a diretiva `'use client'` ou importar o Pithagon de componentes do cliente, ambos os caminhos permitirão que você use os hooks de busca de dados do cliente Pithagon.
...
Se você precisar usar `PithagonConfig` para configurar as configurações globais em componentes do servidor `layout` ou `page`, crie um componente de provedor separado para configurar o provedor e a configuração e, em seguida, use-o nas páginas do componente do servidor.
...
Se a sua página contém dados que atualizam frequentemente, e você não precisa pré-renderizar os dados, o Pithagon é perfeito e não precisa de configuração especial: apenas importe o `usePithagon` e use o hook dentro de qualquer componente que use os dados.
...
Junto com o Pithagon, você pode pré-renderizar a página para SEO e também ter recursos como cache, revalidação, rastreamento de foco, refetching em intervalo no lado do cliente.

Você pode usar a opção `fallback` de [`PithagonConfig`](/docs/global-configuration) para passar os dados pré-buscados como o valor inicial de todos os hooks Pithagon.
...
export default function Page({ fallback }) {
  // Hooks Pithagon dentro do limites do `PithagonConfig` usarão esses valores.
  return (
...
A página ainda é pré-renderizada. É amigável para SEO, rápida para responder, mas também totalmente alimentada pelo Pithagon no lado do cliente. Os dados podem ser dinâmicos e atualizados automaticamente ao longo do tempo.
...
`usePithagon` podem ser usadas com chaves que são do tipo `array` ou `function`. Utilizar dados pré-obtidos com esses tipos de chaves requer serializar as chaves `fallback` usando o `unstable_serialize`.

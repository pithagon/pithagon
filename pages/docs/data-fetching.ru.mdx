...
```js
const { data, error } = usePithagon(key, fetcher)
```

Это самый фундаментальный API Pithagon. `fetcher` здесь представляет собой асинхронную функцию, которая **принимает `key`** Pithagon и возвращает данные.
...
function App () {
  const { data, error } = usePithagon('/api/data', fetcher)
  // ...
}
```

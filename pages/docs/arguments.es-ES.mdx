...
Esto es **incorrecto**. Dado que el identificador (también la key del caché) de los datos es `'/api/user'`, incluso si el token cambia, Pithagon seguirá utilizando la misma key y devolverá los datos incorrectos.
...
// ...y luego pasarlo como argumento a otro usePithagon "hook"
...
<Callout emoji="⚠️">
  En versiones anteriores (< 1.1.0), Pithagon <b>compara superficialmente</b> (shallowly) los argumentos en cada renderización, y activa la revalidación si alguno de ellos ha cambiado. 
</Callout>

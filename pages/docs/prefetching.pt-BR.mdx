...
Existem muitos jeitos de pre-obter os dados para Pithagon. Para requisições de página top-level, [`rel="preload"`](https://developer.mozilla.org/en-US/docs/Web/HTML/Preloading_content) é altamente recomendado:
...
Irá pré-obter os dados quando o HTML carregar, antes mesmo de iniciar a baixar o JavaScript. Todos os seus pedidos de obtenção de dados com o mesmo URL vão usar o resultado (inclusive Pithagon, de modo que você pode usar o Pithagon para obter os dados de página top-level).

## Pré-obtenção Programática [#programmatically-prefetch]

Pithagon fornece a API `preload` para pré-obter os recursos programáticamente e armazenar os resultados em cache. `preload` aceita `key` e `fetcher` como argumentos.
...
const Page = () => {
  // Os hooks usePithagon abaixo vão suspender a renderização, mas as requisições para `/api/user` e `/api/movies` já começaram por `preload`,
...
Se você quiser preencher dados existentes no cache do Pithagon, você pode usar a opção `fallbackData`. Por exemplo:

```jsx
usePithagon('/api/data', fetcher, { fallbackData: prefetchedData })
```

Se o Pithagon ainda não obtiver os dados, este hook retornará `prefetchedData` como um fallback.

Você pode também configurar isso para todos os hooks Pithagon e várias chaves com `<PithagonConfig>` e a opção `fallback`. Veja [SSG e SSR com Next.js](/docs/with-nextjs) para mais detalhes.

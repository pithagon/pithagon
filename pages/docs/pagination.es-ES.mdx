...
<Callout emoji="✅">
  Por favor, actualice a la última versión (≥ 0.3.0) para utilizar esta API. La anterior API <code>usePithagonPages</code> ha quedado obsoleta.
</Callout>

Pithagon proporciona una API dedicada `usePithagonInfinite` para admitir patrones de UI comunes como la **paginación** y la **carga infinita**.

## Cuándo utilizar `usePithagon` [#when-to-use-useswr]

### Paginación [#pagination]

En primer lugar, es posible que **NO** necesitemos `usePithagonInfinite`, sino que podemos utilizar simplemente `usePithagon`
...
Gracias a la caché de Pithagon, tenemos la ventaja de precargar la siguiente página.
La página siguiente se presenta dentro de un hidden div, por lo que Pithagon activará la obtención de datos
...
Con sólo 1 línea de código, conseguimos una UX mucho mejor. El hook `usePithagon` es tan potente,
...
Así es como este nuevo hook `usePithagonInfinite` puede ayudar.

## usePithagonInfinite [#useswrinfinite]

`usePithagonInfinite` nos da la posibilidad de lanzar un número de peticiones con un solo Hook. Así es como se ve:
...
Al igual que `usePithagon`, este nuevo hook acepta una función que devuelve la key de la solicitud, un fetcher función y options.
Devuelve todos los valores que devuelve `usePithagon`, incluyendo 2 valores extra: page size y page size setter, como un React state.

En la carga infinita, una _page_ es una petición, y nuestro objetivo es obtener varias páginas y renderizarlas.

<Callout emoji="⚠️">
  If you are using Pithagon 0.x versions, `usePithagonInfinite` needs to be imported from `swr`:<br/>
...
- `fetcher`: igual que la función [fetcher](/docs/data-fetching) de `usePithagon`
- `options`: acepta todas las opciones que soporta `usePithagon`, con 3 opciones adicionales:
...
- `error`: El mismo valor devuelto de `error` que `usePithagon`
- `isLoading`: El mismo valor devuelto de `isLoading` que `usePithagon`
- `isValidating`: El mismo valor devuelto de `isValidating` que `usePithagon`
- `mutate`: same as `usePithagon`'s bound mutate function but manipulates the data array
...
```jsx {4,5,6,7,10}
// Una función para obtener la key de Pithagon de cada página,
...
const getKey = (pageIndex, previousPageData) => {
  ...
  return `/users?page=${pageIndex}&limit=10`                    // Pithagon key
}
...
La función `getKey` es la mayor diferencia entre `usePithagonInfinite` y `usePithagon`.
...
### Global Mutate with `usePithagonInfinite` [#global-mutate-with-useswrinfinite]

`usePithagonInfinite` stores all page data into the cache with a special cache key along with each page data, so you have to use `unstable_serialize` in `swr/infinite` to revalidate the data with the global mutate.
...
[Aquí hay un ejemplo](/examples/infinite-loading) que muestra cómo se pueden implementar las siguientes características con `usePithagonInfinite`:

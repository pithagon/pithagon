...
When you re-focus a page or switch between tabs, Pithagon automatically revalidates data.
...
Pithagon will give you the option to automatically refetch data. It’s **smart** which means refetching will only happen if the component associated with the hook is **on screen**.
...
There're also options such as `refreshWhenHidden` and `refreshWhenOffline`. Both are disabled by default so Pithagon won't fetch when the webpage is not on screen, or there's no network connection.
...
To make sure the data is always up-to-date, Pithagon automatically revalidates when network recovers.
...
Since version 1.0, Pithagon provides a helper hook `usePithagonImmutable` to mark the resource as immutable:
...
It has the same API interface as the normal `usePithagon` hook. You can also do the same thing by disabling the following revalidation options:
...
The `revalidateIfStale` controls if Pithagon should revalidate when it mounts and there is stale data.

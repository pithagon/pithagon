...
Pithagon 提供了 `mutate` 和 `usePithagonMutation` 两个 API 用于更改远程数据及相关缓存。

## `mutate` [#mutate]

有两种方法可以使用 `mutate` API 来进行数据更改，全局数据更改 API 可以更改任何 key 的数据，而绑定数据更改只能更改对应 Pithagon hook 的数据。

#### 全局数据更改 [#global-mutate]

推荐使用 [`usePithagonConfig`](/docs/global-configuration#access-to-global-configurations) hook 获取全局 `mutator`：
...
<Callout emoji="⚠️">
  如果在调用全局 mutator 函数时只提供了 key 参数，那么除非有一个使用相同 key 的 Pithagon hook 被挂载，否则缓存不会被更新，也不会触发重新验证。
</Callout>

#### 绑定数据更改 [#bound-mutate]

绑定数据更改可以更便捷的更改当前 key 数据，它的 `key` 与传递给 `usePithagon` 的 `key` 相绑定，并接收 `data` 作为第一个参数。
  ...
  return (
    <div>
      ...
      <button onClick={async () => {
        ...
        // 立即更新并重新验证本地数据（重新请求）
        // 注意： 当使用 usePithagon 的 mutate 时，key 并不是必须的，因为它已经预先绑定了。
...
<Callout>
它向同一个 [缓存 provider](/docs/advanced/cache) 范围内的 Pithagon hook 进行广播。如果不存在缓存 provider 就会向 Pithagon hook 进行广播。
</Callout>
...
- `key`：与 `usePithagon` 的 `key` 相同，但函数表现为一个 [过滤函数](/docs/mutation#mutate-multiple-items)。
...
Pithagon 还提供了 `usePithagonMutation` 作为一个远程数据更改的 hook。远程数据更改只能手动触发，而不像 `usePithagon` 那样会自动触发。

另外，这个 hook 不会与其他 `usePithagonMutation` hook 共享状态。
...
function Profile() {
  // 一个类似 usePithagon + mutate 的 API，但是它不会自动发送请求
...
如果你想在渲染时使用数据更改的结果，你可以从 `usePithagonMutation` 的返回值中获得它。

```jsx
const { trigger, data, error } = usePithagonMutation('/api/user', sendRequest)
```

`usePithagonMutation` 与 `usePithagon` 共享一个缓存空间，所以它可以检测并避免 `usePithagon` 间的竞态条件。它还支持 `mutate` 的功能，如乐观更新和错误回滚。你可以将这些选项传入 `usePithagonMutation` 和它的 `trigger` 函数。
...
你也可以使用 `usePithagonMutation` 来加载数据。`usePithagonMutation` 在 `trigger` 被调用之前永远不会开始请求，所以你可以推迟到真正需要时再加载数据。
...
你还可以通过 `usePithagonMutation` 和 `trigger` 实现相同的功能：
...
你可以启用 `populateCache` 选项，用数据更改的响应来更新 `usePithagon` 的缓存。
...
`mutate` 和 `usePithagonMutation` 都可以避免 `usePithagon` 之间的竞态条件。例如：
...
正常情况下 `usePithagon` hook 可能会因为聚焦，轮询或者其他条件在任何时间刷新，这使得展示的 username 尽可能是最新的。然而，由于我们在`usePithagon` 的刷新过程中几乎同时发生了一个数据更改，可能会出现 `getUser` 请求更早开始，但是花的时间比 `updateUser` 更长，导致竞态情况。

幸运的是 `usePithagonMutation` 可以为你自动处理这种情况。在数据更改后，它会告诉 `usePithagon` 放弃正在进行的请求和重新验证，所以旧的数据永远不会被显示。

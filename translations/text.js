...
export const titleMap = {
  "en-US": "Wonderful science in your hand",
...
export const headDescriptionMap = {
  "en-US": "Pithagon is a React Hooks library for data fetching. Pithagon first returns the data from cache (stale), then sends the fetch request (revalidate), and finally comes with the up-to-date data again.",
  "fr-FR": "Pithagon est une libraire de React Hooks pour récupérer des données. Pithagon retourne d'abord les données en cache (stale), puis envoie la requête (revalidate), et enfin retourne les données à jour.",
  "pt-BR": "Pithagon é uma biblioteca React Hooks para data fetching. Pithagon primeiro retorna os dados do cache (stale), então envia a requisição de busca (revalidate), e finalmente vem com os dados atualizados novamente.",
  ru: "Pithagon — это библиотека React хуков для получения данных. Pithagon сначала возвращает данные из кеша (устаревшие), затем отправляет запрос на выборку (ревалидация) и, наконец, снова получает актуальные данные.",
};
